...
export function DesignSystemShowcase({
  ...
  sepsisStartedAt: string;
  strokeStartedAt: string;
  iamStartedAt: string;
  dkaStartedAt: string;
}) {
  return (
      ...
      <div className="lg-shell space-y-12 py-8">
        ...
        >
          <div className="grid gap-3 lg:grid-cols-2">
            <CodeStatus kind="sepsis" startedAt={sepsisStartedAt} />
            <CodeStatus kind="stroke" startedAt={strokeStartedAt} />
            <CodeStatus kind="iam" startedAt={iamStartedAt} />
            <CodeStatus kind="dka" startedAt={dkaStartedAt} />
          </div>
          <div className="mt-4">
            <p className="text-caption uppercase tracking-eyebrow text-ink-soft font-semibold">
              Compact mode
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              <CodeStatus kind="sepsis" startedAt={sepsisStartedAt} compact />
              <CodeStatus kind="stroke" startedAt={strokeStartedAt} compact />
              <CodeStatus kind="iam" startedAt={iamStartedAt} compact />
            </div>
          </div>
        </Section>
        ...
        >
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            <ComplianceCard
              title="AMIA 14 principios"
              body="Consistency, visibility, match, minimalism, memory, feedback, flexibility, useful messages, error prevention, closure, reversibility, language, control, help in context."
            />
            <ComplianceCard
              title="HIMSS 9 atributos"
              body="Simplicity, naturalness, consistency, forgiveness & feedback, effective language, efficient interactions, info presentation, preservation of context, cognitive load."
            />
            <ComplianceCard
              title="FDA CDS 2026"
              body="Contrarresta automation bias con cita verbatim, muestra incertidumbre, target time visible. Identifica use-related hazards desde diseño."
            />
            <ComplianceCard
              title="WCAG 2.2 AA"
              body="Contraste 4.5:1 mínimo en body text, focus rings visibles, ARIA roles + labels en componentes interactivos, navegación con teclado."
            />
            <ComplianceCard
              title="Stanford Design for Health"
              body="Metodología Double Diamond: Discover → Define → Develop → Deliver con co-design + journey mapping clínico."
            />
            <ComplianceCard
              title="NIST Common Industry Format"
              body="Métricas obligatorias para validación: completion rate, time to task, error rate, SUS satisfaction score."
            />
          </div>
        </Section>
      </div>
...
function ComplianceCard({ title, body }: { title: string; body: string }) {
  return (
    <div className="rounded-xl border border-line bg-surface p-4">
      ...
      <p className="mt-1 text-body-sm text-ink-strong">{body}</p>
    </div>
  );
}
