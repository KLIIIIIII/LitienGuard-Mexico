...
export function RadiologiaCliente({
  estudios,
  eventos,
}: {
      ...
      <section className="lg-card space-y-4">
        ...
        <Field label="Indicación clínica">
          <textarea
            ...
            placeholder="Ej. Cefalea súbita de máxima intensidad, sospecha HSA"
...
function PeticionRow({
  ...
  function guardar() {
    ...
    startTransition(async () => {
      await adjuntarReporte({
        eventoId: evento.id,
        reporte: reportText.trim(),
      });
      setShowReport(false);
    });
  }
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-caption font-semibold text-ink-strong">
              {nEstudios} {nEstudios === 1 ? "estudio" : "estudios"}
        ...
        {activa && (
          ...
          >
            Adjuntar reporte <ChevronRight className="inline h-3 w-3" />
          </button>
        )}
      </div>
      ...
      {activa && showReport && (
        <div className="space-y-2">
          <textarea
            value={reportText}
            onChange={(e) => setReportText(e.target.value.slice(0, 3000))}
            placeholder="Pega o escribe el reporte estructurado…"
            className="lg-input min-h-[120px] resize-y w-full"
          />
          <div className="flex justify-end gap-2">
            ...
            <button
              type="button"
              onClick={guardar}
              disabled={pending || reportText.trim().length < 10}
